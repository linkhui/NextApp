...
@implementation NARouter{
    NSMutableDictionary  *_routerMap;
}
+ (instancetype)defaultRouter {
    static dispatch_once_t onceToken;
    static NARouter *router;
...
- (NARouterResult *)route:(NSString *)urlString withParam:(NSDictionary *)param {
    ...
    NARouterResult *result = [self checkSchemeTransferWithUrl:urlString withParam:param fromViewController:sourceViewController];
    return result;
}

- (NARouterResult *)impRoute:(NSString *)urlString withParam:(NSDictionary *)param fromViewController:(UIViewController *)viewController {
    return [[NARouterImp instance] route:urlString parameters:param];
}
...
- (BOOL)couldRouteBySchemeHandler:(NSString *)urlString {
    ...
    if (schemeURLString.length) {
        ...
        NARouterImp *routerImp = _routerMap[scheme];
...
- (NARouterResult *)route:(NSString *)urlString withParam:(NSDictionary *)param fromViewController:(UIViewController *)viewController {
    // TODO: 跳转
    return [self checkSchemeTransferWithUrl:urlString withParam:param fromViewController:viewController];
}

- (NARouterResult *)routeToWeexWithUrlString:(NSString *)urlString
...
// 检查H5 to native转换
- (NARouterResult *)checkSchemeTransferWithUrl:(NSString *)urlString withParam:(NSDictionary *)param fromViewController:(UIViewController *)viewController {
...
- (NARouterImp *)routerForScheme:(NSString *)scheme{
    NARouterImp *imp = nil;
    if (scheme == nil) {
         scheme = @"nextapp";
    }
    ...
    if (!imp) {
        imp = [[NARouterImp instance]newSubRouter:[NSString stringWithFormat:@"%@://",scheme]];
